...
이 서버는 CPU 전용이다. GPU(AMD Radeon RX 7800 XT, ROCm) 서버는 별도로 있고
Whisper large-v3·CosyVoice3·XTTS-v2·Qwen3-TTS·Fish Speech·Zipformer·MOSS-TTS-Nano
7종을 돌린다. 설치·ROCm 제약·엔진별 실측치는 `HANDOFF-GPU.md` 를 볼 것.
...
**(GPU 서버(WSL)) `docker compose` v2 플러그인이 없다.** `docker-compose`(독립형 v5.1.1)만 설치돼
있어 `voicectl.sh`의 `dc()`가 `docker compose` 대신 `docker-compose`를 호출하도록 고쳐놓았다.
플러그인을 나중에 설치하면 되돌릴 필요는 없다(`docker-compose`가 계속 동작한다).
